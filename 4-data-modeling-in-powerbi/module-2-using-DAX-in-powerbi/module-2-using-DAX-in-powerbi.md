...
## 🧪 10. Practical Scenario Patterns (Generalized)

These examples show how the concepts in this module can be applied in professional settings. Names and companies are generalized.

### 10.1 Profitability Analysis (Finance / Data Analyst)

Objective:

- Analyze profit and profit margin by product category and year.
...
- Use measures for:
  ...
  - Overall profit margin at different levels (year, region, category)

---

### 10.2 Customer Segmentation (Marketing Analyst)

Objective:

- Segment customers based on spending or behavior.

Technical approach:

- Use measures and calculated tables to:
  - Sum total sales per customer  
  - Use `DISTINCTCOUNT` and `SUM` to derive metrics per segment  
  - Build segments (e.g., High/Medium/Low value) using `IF` and thresholds  

---

### 10.3 Time-Based Performance Tracking (Sales / Operations Analyst)

Objective:

- Track performance by period and compare against previous periods.

Technical approach:

- Create a date table using `CALENDAR` or `CALENDARAUTO`  
- Use time intelligence functions like `SAMEPERIODLASTYEAR`, `DATESYTD`  
- Define measures for:
...
### 10.4 Inventory and Balance Reporting (Inventory / Finance Analyst)

Objective:

- Monitor inventory or account balances at specific points in time.

Technical approach:

- Use semi-additive measure patterns:
  - Use snapshot logic for end-of-month or end-of-day values  
  - Avoid summing balances across time  
- Use `CALCULATE` with time intelligence to pick the correct date.

---

## ✅ 11. Best Practices for DAX

- **Use measures where possible** for aggregations  
- **Reserve calculated columns** for row-level attributes, keys, and groupings  
- **Use variables (`VAR` / `RETURN`)** to simplify complex logic  
- **Format DAX with whitespace** for readability and easier debugging  
- **Test and validate** calculations with known values and edge cases  
- **Design relationships intentionally** to control filter propagation and avoid ambiguity  
...
- Explain what DAX is and where it’s used  
- Understand DAX syntax, data types, and operators  
- Differentiate between row context and filter context, and understand how they interact  
- Create and use **calculated tables**, **calculated columns**, and **measures**  
- Distinguish between **additive**, **semi-additive**, and **non-additive** measures  
- Apply statistical and time intelligence functions for real-world analysis  
- Follow best practices to write clean, efficient, and maintainable DAX

This knowledge forms the core of building robust, analytical data models in Power BI that support reliable, insight-rich reporting.
